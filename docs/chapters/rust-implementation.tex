...
This closely matches the quotient-filter model: split the hash into quotient/remainder, then either create a new run or append into an existing run while preserving \texttt{occupied}, \texttt{continuation}, and \texttt{shifted} metadata.
...
Our implementation performs \textbf{only Phase 1}. The relevant code is as follows:
...
We place a tombstone at the canonical slot where the query found the match, but we do not track down or remove any of its duplicates. This has two concrete consequences:
...
This section documents exactly what matches and what was intentionally simplified compared to the official Rust implementation at \texttt{github.com/nivdayan/AlephFilter} (branch \texttt{aleph\_filter}), to make the scope of this implementation clear.
