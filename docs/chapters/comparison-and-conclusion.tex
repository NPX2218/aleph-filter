\chapter{Comparison and Conclusion}
  ...
  Queries keys that are known to be in the filter (should all return \texttt{true}).
  ...
  Queries keys that are \emph{not} in the filter. This measures the common-case
  ...
  \begin{insightbox}[Lookup Analysis]
    ...
    At $N = 100{,}000$, Aleph's quotient-filter structure (linear probing clusters) shows
  ...
  \begin{warningbox}[Expansion Cost]
    Starting from a 64-slot filter, the amortized insert cost with expansion (195 ns/op)
    is $\sim$13$\times$ slower than pre-sized insertion (15 ns/op). This overhead comes from
